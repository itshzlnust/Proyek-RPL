        ...
        <form action="{{ route('treatments.store') }}" method="POST">
            ...
            <div class="row mb-3">
                ...
                <div class="col-md-6">
                    ...
                    <input type="number" class="form-control @error('price') is-invalid @enderror" id="price" name="price" min="0" step="1000" value="{{ old('price') }}" required>
                    @error('price')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
            ...
            <div class="mb-3">
                <div class="form-check">
                    <input class="form-check-input" type="checkbox" id="is_bundle" name="is_bundle" value="1" {{ old('is_bundle') ? 'checked' : '' }}>
            ...
            <div class="mb-3" id="bundle_name_field" style="{{ old('is_bundle') ? '' : 'display:none;' }}">
                <label for="bundle_name" class="form-label">Bundle Name</label>
                <input type="text" class="form-control @error('bundle_name') is-invalid @enderror" id="bundle_name" name="bundle_name" value="{{ old('bundle_name') }}">
                @error('bundle_name')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
