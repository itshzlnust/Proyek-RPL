...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Add initial row if we have flash data (coming back after validation error)
        @if(old('treatments'))
            // Hide empty row
            emptyRow.style.display = 'none';
            
            @foreach(old('treatments') as $index => $treatment)
                // Add a new row with the old data
                const clone = document.importNode(treatmentRowTemplate.content, true);
                const row = clone.querySelector('tr');
                ...
                // Set the values
                const treatmentSelect = row.querySelector('.treatment-select');
                treatmentSelect.value = '{{ $treatment['treatment_id'] }}';
                ...
                treatmentIndex = Math.max(treatmentIndex, {{ $index }} + 1);
            @endforeach
